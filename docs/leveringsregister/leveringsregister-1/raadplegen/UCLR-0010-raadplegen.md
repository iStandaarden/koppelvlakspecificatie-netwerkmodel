# Raadplegen van de Behandelingperiode door het zorgkantoor (UCLR-0010) 
...
flowchart LR
 subgraph s1["Leveringsregister"]
          B["Raadplegen Behandelingperiode"]
  end
    A["Zorgkantoor"] --> B
    B@{ shape: terminal}
    A@{ shape: rounded}
...
**Titel:** Raadplegen van de behandelingperiode (na notificatie) door het zorgkantoor  
**Actoren:** Zorgkantoor die betrokken is bij de levering van zorg aan een client.  

### Precondities:
- De behandelingperiode is opgenomen in het Leveringsregister.
...
### Autorisatie:
Het zorgkantoor mag de behandelingperiode (en overige gegevens) raadplegen. 
...
**Trigger:**
- Een zorgkantoor wil de (actuele) behandelingperiode (en overige informatie) raadplegen waarvan het de notificatie heeft ontvangen.
...
|---|---|---|---|
| [**QLR-0010-ZK**.graphql](https://github.com/iStandaarden/iWlz-levering/tree/Leveringsregister-1/gql-query/zorgkantoor/QLR-0010-ZK.graphql) | Op basis van de (ontvangen) behandelingperiodeID en eigen identificatie, de behandelingperiode en overig toegestane informatie raadplegen | `behandelingperiodeID`; `bemiddelingspecificatieID` |  Behandelingperiode / Leveringperiode / Levering /  Client / Uitstelperiode / Afstel |

## **Proces raadplegen**

Een zorgkantoor is (via een zorgaanbieder) betrokken bij de zorg van een client. Als de zorgaanbieder een behandelingperiode registreert, aanpast of verwijdert, ontvangt het zorgkantoor daarvan een notificatie. (Ga naar het overzicht [notificaties](/notificaties#notificaties-aan-het-zorgkantoor) om te bekijken welke dit zijn).
...
stateDiagram
  ...
  raadplegen:Raadplegen Leveringsregister voor (actuele) status behandelingperiode
  idAvailable:behandelingperiodeID en bemiddelingspecificatieID bekend?
  notifyWait:Wacht op notificatie
  QBR0001ZAiq:Gebruik behandelingperiodeID en bemiddelingspecificatieID
  notifyReceive:notificatie NIEUWE_BEHANDELINGPERIODE_ZORGKANTOOR of GEWIJZIGDE_BEHANDELINGPERIODE_ZORGKANTOOR ontvangen
  QBR0001ZA:Gebruik template QLR-0010-ZK
...
| 2. | Zijn het **`behandelingperiodeID`** en **`bemiddelingspecificatieID`** bekend? <br/><ol><li> - **Ja** →  Ga verder naar stap 6 <br/><li> - **Nee** → Wacht op notificatie [**`NIEUWE_BEHANDELINGPERIODE_ZORGKANTOOR`** of **`GEWIJZIGDE_BEHANDELINGPERIODE_ZORGKANTOOR`**](../notificaties/)  | 
| 4. | Notificatie is ontvangen | 
| 5. | Gebruik de informatie uit de notificatie voor het raadplegen van het leveringsregister |
| 6. | Het zorgkantoor vult de verplichte **`behandelingperiodeID`** en **`bemiddelingspecificatieID`** in query-template [QLR-0010-ZK.graphqlgraphql](https://github.com/iStandaarden/iWlz-levering/tree/Leveringsregister-1/gql-query/zorgkantoor/QLR-0010-ZK.graphql) en initieert een raadpleging van de leveringperiode in het Leveringsregister. | 
| 7. | Het zorgkantoor stuurt Graphql-request + Access-token naar het Policy Enforcement Point (PEP) |
| 8. | De PEP voert de [toegangscontrole](../toegangscontrole/UCLR-0010-toegangscontrole.md) uit en stuurt bij toegang het request door naar het Leveringsregister. |
