# Toegangscontrole: Raadplegen van de Overdracht door het nieuw verantwoordelijk zorgkantoor (UCBR-0007)  
...
- **Omschrijving:** Uitvoeren van GraphQL-query [`QBR-0007-ZKn.graphql`graphql](https://github.com/iStandaarden/iWlz-bemiddeling/tree/Bemiddelingsregister-1_1/gql-query/zorgkantoor/QBR-0007-ZKn.graphql) op het bemiddelingsregister door een zorgkantoor.


### **Resource**
- **Type:** `Bemiddelingsregister`
- **ID:** `overdrachtID`
- **Beperking:** Alleen toegang tot gegevens waarvoor het zorgkantoor een Overdracht heeft (op basis van uzovicode en overdrachtID)
- **Inhoud:** Alleen de nodes overdracht en de gerelateerde Bemiddeling en Overdrachtspecificatie en Bemiddelingspecificatie die horen bij de opgevraagde Overdracht, mogen direct worden opgevraagd.


### **Context**
- **Query-parameters vereist:** De `overdrachtID` en de `uzovicode` moeten aanwezig zijn in de query
...
> Toegang tot het Bemiddelingsregister via query [`QBR-0007-ZKn.graphql`graphql](https://github.com/iStandaarden/iWlz-bemiddeling/tree/Bemiddelingsregister-1_1/gql-query/zorgkantoor/QBR-0007-ZKn.graphql) is **alleen toegestaan** als:
>
...
> - De in de query meegegeven `uzovicode` komt overeen met de `uzovicode` in de access-token; 
> 
> Als aan alle voorwaarden is voldaan, mogen de nodes `Overdrachtspecificatie`, `Bemiddeling`, `Bemiddelingspecificatie` en `Client` die horen bij deze `Overdracht` direct worden opgevraagd.


## Toegangscontrole-flows Zorgkantoor: QBR-0007-ZKn.graphql
...
stateDiagram
  ...
  indienen: Ontvang QBR-0007-ZKn + Access token
