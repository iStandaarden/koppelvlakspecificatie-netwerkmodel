 ...
 * en koppelt svg-pan-zoom (slepen = verschuiven; scrollen doet niets, zodat
 * de pagina normaal blijft scrollen over een diagram heen) plus een
 * fullscreen-knop aan elk diagram. Klikken op het diagram vergroot het
 * (dezelfde actie als de fullscreen-knop).
 *
...
(function () {
  ...
  function maakFullscreenKnop(blok, herpas) {
    ...
    function isVergroot() {
      return document.fullscreenElement === blok || blok.classList.contains("mermaid-zoom-blok--maxi");
    }

    function toggleVergroot() {
    ...
    knop.isVergroot = isVergroot;
    knop.vergroot = toggleVergroot;
    return knop;
  }
  ...
  function toonDiagram(blok, svgTekst) {
    ...
    hint.textContent = "klik = vergroten · slepen = verschuiven";
    ...
    var knop = maakFullscreenKnop(blok, herpas);
    ...
    /* Klikken op het diagram vergroot het, tenzij het al vergroot is of de
       gebruiker aan het slepen was (svg-pan-zoom gebruikt ook mousedown/
       mouseup voor pannen, dus alleen een klik zonder noemenswaardige
       muisbeweging telt als "klik"). */
    var muisStart = null;
    var DRAG_DREMPEL = 5;

    stage.addEventListener("pointerdown", function (e) {
      muisStart = { x: e.clientX, y: e.clientY };
    });
    stage.addEventListener("pointerup", function (e) {
      if (!muisStart) return;
      var dx = e.clientX - muisStart.x;
      var dy = e.clientY - muisStart.y;
      muisStart = null;
      if (Math.hypot(dx, dy) > DRAG_DREMPEL) return; /* was slepen, geen klik */
      if (knop.isVergroot()) return; /* al vergroot: laat interactie met de popup met rust */
      if (e.target.closest(".mermaid-zoom-blok__fs, .svg-pan-zoom-control")) return;
      knop.vergroot();
    });
  }
